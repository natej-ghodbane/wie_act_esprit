  ...
  useEffect(() => {
    ...
    const fetchVendors = async () => {
      try {
        ...
        const { data } = await marketplaceAPI.getAll(); // axios: data is already array
        ...
        const mapped: VendorCardData[] = Array.isArray(data)
          ? data.map((m: any) => ({
              ...
              image: m.bannerImage || m.logo || "/placeholder.png",
              location: m.location?.city || "Unknown location",
              productsCount: m.products?.length ?? m.productsCount ?? 0, // fallback
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        {!loading &&
          vendors.map((v, idx) => (
            <Link
              key={v.slug}
              href={`/marketplace/vendor/${v.slug}`}
              className="group"
            >
              <article
